...
import { readingApi, readingPartsApi } from "../services/api";
import { ReadingPart, DifficultyLevel } from "../types";
import type {
  IeltsReading,
  IeltsReadingPart,
...
export default function ReadingBuilderPage() {
  ...
  const [parts, setParts] = useState<IeltsReadingPart[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editingPartId, setEditingPartId] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const emptyPartForm = (): CreateReadingPartDto => ({
    reading_id: id!,
  ...
  const [partForm, setPartForm] =
    useState<CreateReadingPartDto>(emptyPartForm());
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [readingData, partsData] = await Promise.all([
        readingApi.getById(id!),
        readingPartsApi.getAll({ readingId: id }),
      ]);
  ...
  const handleCreatePart = async () => {
    setSaving(true);
    try {
      await readingPartsApi.create(partForm);
  ...
  const handleUpdatePart = async () => {
    ...
    setSaving(true);
    try {
      await readingPartsApi.update(editingPartId, partForm);
  ...
  const startEditPart = async (partId: string) => {
    try {
      const part = await readingPartsApi.getById(partId);
      setPartForm({
        reading_id: id!,
        part: part.part,
        title: part.title || "",
  ...
  const handleDeletePart = async (partId: string) => {
    ...
    try {
      await readingPartsApi.delete(partId);
  ...
  const availableParts = Object.values(ReadingPart).filter(
    (p) =>
  ...
  if (!reading)
    return (
      <div className="page">
        <div className="empty-state">Reading not found</div>
      </div>
    );
        ...
        {parts.map((part) => (
          <div key={part.id} className="part-card">
            ...
            {part.questions && part.questions.length > 0 && (
              <div className="part-card-questions">
                <span>{part.questions.length} question group(s)</span>
              </div>
            )}
          </div>
        ))}
        ...
        <div className="form-card part-form">
            ...
            <div className="form-group" style={{ flex: "0 0 150px" }}>
              ...
              <select
                ...
                onChange={(e) =>
                  setPartForm({
                    ...partForm,
                    part: e.target.value as ReadingPart,
                  })
                }
              >
          ...
          <div className="form-group">
            ...
            <textarea
              ...
              onChange={(e) =>
                setPartForm({ ...partForm, content: e.target.value })
              }
          ...
          <QuestionBuilder
            ...
            onChange={(questions: CreateReadingPartQuestionDto[]) =>
              setPartForm({
                ...partForm,
                questions,

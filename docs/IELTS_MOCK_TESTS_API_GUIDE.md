...
```javascript
const response = await fetch('/api/ielts-mock-tests/my', {
...
```javascript
// Mark listening as finished
await fetch(`/api/ielts-mock-tests/${mockTestId}`, {
  method: 'PATCH',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${token}`
...
### Archive a completed mock test

```javascript
await fetch(`/api/ielts-mock-tests/${mockTestId}/archive`, {
  method: 'PATCH',
  headers: { 'Authorization': `Bearer ${token}` }
});
```

### Filter mock tests by group

```javascript
const response = await fetch('/api/ielts-mock-tests/group/group-uuid', {
  headers: { 'Authorization': `Bearer ${token}` }
});
const groupTests = await response.json();
```

...
import { Controller, Get, Post, Body, Patch, Param, Delete, UseGuards, Query, ParseUUIDPipe, HttpCode, HttpStatus, BadRequestException, } from "@nestjs/common";
...
let StudentPaymentController = class StudentPaymentController {
    ...
    create(paymentData, user) {
        const createStudentPaymentDto = {
            ...paymentData,
    ...
    async checkDuePayments() {
        try {
            ...
            throw new BadRequestException(`Error checking due payments: ${error.message}`);
        }
    }
    async checkDuePaymentsLegacy() {
        try {
            return await this.studentPaymentService.checkDuePayments();
        }
        catch (error) {
...
__decorate([
    ...
    __metadata("design:paramtypes", [CreateStudentPaymentRequestDto, Object]),
...
__decorate([
    ...
    ApiOperation({
        summary: "Check which payments would be processed by the automatic payment job",
    }),
    ApiResponse({
        status: 200,
...
__decorate([
    ...
    Roles(Role.ADMIN, Role.TEACHER),
    ApiOperation({
        summary: "Legacy endpoint - Check which payments would be processed by the automatic payment job",
        deprecated: true,
    }),
    ApiResponse({
        status: 200,
    ...
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], StudentPaymentController.prototype, "checkDuePaymentsLegacy", null);

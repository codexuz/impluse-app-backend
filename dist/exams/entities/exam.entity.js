var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Table, Column, Model, DataType, CreatedAt, UpdatedAt, DeletedAt, } from "sequelize-typescript";
let Exam = class Exam extends Model {
};
__decorate([
    Column({
        type: DataType.UUID,
        defaultValue: DataType.UUIDV4,
        primaryKey: true,
    }),
    __metadata("design:type", String)
], Exam.prototype, "id", void 0);
__decorate([
    Column({
        type: DataType.STRING,
        allowNull: false,
    }),
    __metadata("design:type", String)
], Exam.prototype, "title", void 0);
__decorate([
    Column({
        type: DataType.UUID,
        allowNull: false,
    }),
    __metadata("design:type", String)
], Exam.prototype, "group_id", void 0);
__decorate([
    Column({
        type: DataType.DATE,
        allowNull: false,
    }),
    __metadata("design:type", Date)
], Exam.prototype, "scheduled_at", void 0);
__decorate([
    Column({
        type: DataType.ENUM("scheduled", "completed", "cancelled"),
        allowNull: true,
        defaultValue: "scheduled",
    }),
    __metadata("design:type", String)
], Exam.prototype, "status", void 0);
__decorate([
    Column({
        type: DataType.BOOLEAN,
        allowNull: true,
    }),
    __metadata("design:type", Boolean)
], Exam.prototype, "is_online", void 0);
__decorate([
    Column({
        type: DataType.ENUM("beginner", 'elementary', "pre-intermediate", "intermediate"),
        allowNull: false,
        defaultValue: "beginner",
    }),
    __metadata("design:type", String)
], Exam.prototype, "level", void 0);
__decorate([
    CreatedAt,
    __metadata("design:type", Date)
], Exam.prototype, "created_at", void 0);
__decorate([
    UpdatedAt,
    __metadata("design:type", Date)
], Exam.prototype, "updated_at", void 0);
__decorate([
    DeletedAt,
    __metadata("design:type", Date)
], Exam.prototype, "deleted_at", void 0);
Exam = __decorate([
    Table({
        tableName: "exams",
        timestamps: true,
        paranoid: true,
    })
], Exam);
export { Exam };
//# sourceMappingURL=exam.entity.js.map
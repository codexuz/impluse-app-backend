...
export class GroupAssignedUnitsService {
  ...
  async create(
    createDto: CreateGroupAssignedUnitDto
    ...
    try {
      ...
      const lessons = await this.lessonService.findByModuleId(
        createDto.unit_id
      );
      ...
      if (lessons && lessons.length > 0) {
        ...
        const endDate = createDto.end_date
          ...
          : new Date(startDate.getTime() + 30 * 24 * 60 * 60 * 1000); // Default to 30 days if not provided
  ...
  async findOne(id: string): Promise<GroupAssignedUnit> {
    ...
    if (!assigned) {
      throw new NotFoundException(
        `Group assigned unit with ID ${id} not found`
      );
    }
  ...
  async update(
    id: string,
    updateDto: UpdateGroupAssignedUnitDto
  ): Promise<GroupAssignedUnit> {
    ...
    if (updateDto.start_date || updateDto.end_date) {
      try {
        ...
        const assignedLessons =
          await this.groupAssignedLessonsService.findByUnitId(id);
        ...
        console.error(
          `Failed to update lesson assignments for unit ${id}:`,
          error
        );
  ...
  async remove(id: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `Failed to remove lesson assignments for unit ${id}:`,
        error
      );

...
export class LeadTrialLessonsController {
  constructor(
    private readonly leadTrialLessonsService: LeadTrialLessonsService
  ) {}
  ...
  async findByLead(@Param("leadId") leadId: string) {
    // Use service method that uses the correct aliases
  ...
  update(
    @Param("id") id: string,
    @Body() updateLeadTrialLessonDto: UpdateLeadTrialLessonDto
  ) {

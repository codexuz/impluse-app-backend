...
export class IeltsListeningController {
  ...
  // ========== Listening ↔ ListeningPart Link ==========
  @Post("link-part")
  @HttpCode(HttpStatus.CREATED)
  @Roles(Role.ADMIN, Role.TEACHER)
  @ApiOperation({
    summary:
      "Link an existing listening part to a listening section (many-to-many)",
  })
  @ApiResponse({
    status: HttpStatus.CREATED,
    description: "Listening part linked successfully.",
  })
  ...
  @Roles(Role.ADMIN, Role.TEACHER)
  @HttpCode(HttpStatus.NO_CONTENT)
  @ApiOperation({ summary: "Unlink a listening part from a listening section" })
  ...
  @ApiOperation({
    summary:
      "Get all listening parts linked to a listening section (many-to-many)",
  })
  @ApiParam({ name: "id", description: "The listening ID" })

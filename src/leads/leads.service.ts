...
export class LeadsService {
  ...
  async getLeadStats(): Promise<{
    ...
    adminPerformance: { adminId: string; leadsCount: number; convertedCount: number; conversionRate: number }[];
  ...
  async getConversionRates(): Promise<{
    ...
    for (let i = 0; i < statuses.length - 1; i++) {
      ...
      // Find counts
      const currentStatusObj = statusTransitions.find((s: any) => s.status === currentStatus);
      const nextStatusObj = statusTransitions.find((s: any) => s.status === nextStatus);
   ...
   */
  async getAdminPerformance(): Promise<{ adminId: string; leadsCount: number; convertedCount: number; conversionRate: number }[]> {
    const sequelize = this.leadModel.sequelize;
   ...
   */
  async getAverageTimeInStatus(): Promise<{ status: string; averageDays: number }[]> {
   ...
   */
  async getLeadStatsByDateRange(startDate: Date, endDate: Date): Promise<{
    totalLeads: number;
    leadsByStatus: { [key: string]: number };
    leadsBySource: { [key: string]: number };

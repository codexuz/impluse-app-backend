...
export class GroupHomeworksService {
  ...
  async getHomeworksForUser(userId: string): Promise<any[]> {
    ...
    // Get all submissions for this user and these homeworks
    ...
    // Create a map of submissions by homework_id and section
    ...
    const homeworksWithStatus = homeworks.map((homework) => {
      ...
      // Add status to speaking exercises
      if (homeworkData.lesson && homeworkData.lesson.speaking) {
        ...
        homeworkData.lesson.speaking = {
          ...homeworkData.lesson.speaking,
          status: speakingSubmission
            ? speakingSubmission.status === "passed" ||
              speakingSubmission.status === "failed"
              ? "finished"
              : "unfinished"
            : "unfinished",
      ...
      // Add status to vocabulary exercises
      if (homeworkData.lesson && homeworkData.lesson.lesson_vocabulary) {
        homeworkData.lesson.lesson_vocabulary =
          homeworkData.lesson.lesson_vocabulary.map((vocab) => {
            ...
            return {
              ...vocab,
              status: vocabSubmission
                ? vocabSubmission.status === "passed" ||
                  vocabSubmission.status === "failed"
                  ? "finished"
                  : "unfinished"
                : "unfinished",
      ...
      // Add overall homework status
      const homeworkSubmissions = submissions.filter(
        (sub) => sub.homework_id === homework.id
      );
      const isHomeworkFinished = homeworkSubmissions.some(

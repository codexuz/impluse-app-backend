...
export class StudentVocabularyProgressService {
  ...
  async create(createDto: CreateStudentVocabularyProgressDto): Promise<StudentVocabularyProgress> {
    // Initialize attempts_count to 1 for new records
  ...
  async getGlobalStats(): Promise<{
    ...
    mostDifficultItems: Array<{ vocabularyItemId: string; difficultyScore: number; masteryRate: number; averageAttempts: number }>;
    mostMasteredItems: Array<{ vocabularyItemId: string; masteryRate: number; totalStudents: number; averageAttempts: number }>;
  }> {
   ...
   */
  async getStudentRankings(limit: number = 10): Promise<Array<{
    studentId: string;
    totalItems: number;
   ...
   */
  async getStudentEfficiencyRankings(limit: number = 10): Promise<Array<{
    studentId: string;
    masteredItems: number;
    averageAttemptsToMastery: number;
    efficiency: number;
